<div class="col-span-1 sm:col-span-2 lg:col-span-3 bg-white p-6 rounded-lg shadow-lg">
    @if (session('success'))
        <div id="success-message" data-message="{{ session('success') }}" data-redirect="{{ route('supplier.index') }}">
        </div>
    @endif
    <form id="add-supplier" action="{{ route('supplier.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div>
                        <label class="block text-gray-700">
                            ...
                            <x-text-input id="nama_supplier" class="block mt-1 w-full" type="text"
                                name="nama_supplier" :value="old('nama_supplier')" required autofocus
